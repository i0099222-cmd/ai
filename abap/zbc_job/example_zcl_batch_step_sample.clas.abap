...
"! ZTBATCH_SCHED-EXEC_CLASS 에 이 클래스 이름을 넣으면 런처가 동적 생성해서 실행한다.
"!
"! 이관 방법:
...
CLASS zcl_batch_step_sample DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.

  PUBLIC SECTION.
    INTERFACES zif_batch_step.
...
CLASS zcl_batch_step_sample IMPLEMENTATION.

  METHOD zif_batch_step~execute.
    ...
    IF iv_param IS NOT INITIAL.
      TRY.
        ...
        CATCH cx_root INTO DATA(lx_parse).
          RAISE EXCEPTION NEW zcx_batch_job(
    ...
    IF ls_param-company_code IS INITIAL.
      RAISE EXCEPTION NEW zcx_batch_job( message = 'Company code is required' ).
    ENDIF.
