...
*&---------------------------------------------------------------------*
*& APJ 런처가 "임의의 리포트"를 실행하기 위한 유일한 통로.
*&
*& SUBMIT 은 ABAP for Cloud Development 에서 금지되므로, 이 FM 만
*& Standard ABAP 언어버전에 두고 Local API 로 release 해서
*& ZCL_APJ_JOB_LAUNCHER(Cloud) 가 호출한다.
*&
*& ** 이 FM 하나가 "APJ 는 임의 프로그램을 못 돌린다"는 제약을 푸는 열쇠다. **
*&
*& SE37 생성:
...
*&            IV_LANGUAGE TYPE SY-LANGU    (선택)
*&   Export : EV_SUBRC    TYPE SY-SUBRC
*&            EV_MESSAGE  TYPE STRING
*&
*&   생성 후 SE37 > Goto > API State > "Use in Cloud Development"(Local API) release
*&
*& NOTE - 실행 사용자
*&   SUBMIT 은 현재 세션 사용자 권한으로 실행된다. SM36 의 스텝별 사용자
*&   (AUTHCKNAM)처럼 스텝마다 다른 사용자로 돌릴 수 없다.
*&   잡 전체가 APJ 를 스케줄한 사용자 컨텍스트로 돈다. (COMPARISON #2)
*&
*& NOTE - 스풀
*&   백그라운드에서 SUBMIT 한 리포트의 WRITE 리스트는 잡의 스풀로 간다.
*&   APJ 잡에서도 스풀이 생기는지는 실측 확인 대상. (COMPARISON #9)
*&---------------------------------------------------------------------*
FUNCTION z_bc_run_report.
  ...
  " 존재하지 않는 프로그램을 SUBMIT 하면 짧은 덤프가 난다. 미리 막는다.
  ...
  TRY.
      ...
      ev_message = COND #(
        WHEN sy-subrc = 0
          THEN |{ iv_program }{ COND string( WHEN iv_variant IS NOT INITIAL
                                             THEN | ({ iv_variant })| ) } executed|
