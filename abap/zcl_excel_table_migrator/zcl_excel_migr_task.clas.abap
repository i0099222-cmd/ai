"! 엑셀 마이그레이션을 RAP LUW 밖에서 실행하기 위한 CL_ABAP_PARALLEL 태스크.
"!
"! RAP은 interaction 단계(액션 핸들러)에서의 DB 변경과 COMMIT WORK를 금지한다.
"! 이 태스크는 별도 세션에서 돌기 때문에 자체 LUW를 가지며, 여기서의 커밋은
"! 정상적으로 허용된다.
"!
"! 대가: 이 커밋은 RAP 트랜잭션과 무관하게 확정된다. 액션 이후 RAP save가
"! 실패하거나 사용자가 취소해도 여기서 넣은 데이터는 롤백되지 않는다.
"! 재업로드 시 중복이 문제가 된다면 대상 테이블에 업로드 식별자를 두고
"! 중복 체크를 거는 것을 고려할 것.
CLASS zcl_excel_migr_task DEFINITION
  PUBLIC
  CREATE PUBLIC.

  PUBLIC SECTION.

    INTERFACES if_abap_parallel.
    INTERFACES if_serializable_object.

    METHODS constructor
      IMPORTING
        iv_tabname      TYPE tabname
        iv_file_content TYPE xstring
        it_keep_initial TYPE string_table OPTIONAL.

    "! do( )가 채우고, run_inst( ) 이후 호출부가 읽어간다.
    "! 별도 세션이라 리턴값으로는 넘길 수 없어 인스턴스 속성을 쓴다.
    DATA ms_result TYPE zcl_excel_table_migrator=>ty_result READ-ONLY.

    "! 예외는 세션 경계를 넘지 못하므로 텍스트로 담아 돌려준다.
    DATA mv_error_text TYPE string READ-ONLY.

  PRIVATE SECTION.

    DATA mv_tabname      TYPE tabname.
    DATA mv_file_content TYPE xstring.
    DATA mt_keep_initial TYPE string_table.

ENDCLASS.


CLASS zcl_excel_migr_task IMPLEMENTATION.

  METHOD constructor.
    mv_tabname      = iv_tabname.
    mv_file_content = iv_file_content.
    mt_keep_initial = it_keep_initial.
  ENDMETHOD.


  METHOD if_abap_parallel~do.

    TRY.

        ms_result = NEW zcl_excel_table_migrator( )->migrate(
          iv_tabname      = mv_tabname
          iv_file_content = mv_file_content
          it_keep_initial = mt_keep_initial ).

        " migrate( )는 기본적으로 전부 아니면 전무라, 넣을 게 있을 때만 확정한다.
        IF ms_result-inserted > 0.
          COMMIT WORK AND WAIT.
        ELSE.
          ROLLBACK WORK.
        ENDIF.

      CATCH cx_root INTO DATA(lx_error).
        " 여기서 안 잡으면 호출부는 아무 단서 없이 빈 결과만 받게 된다.
        ROLLBACK WORK.
        mv_error_text = lx_error->get_text( ).

    ENDTRY.

  ENDMETHOD.

ENDCLASS.
