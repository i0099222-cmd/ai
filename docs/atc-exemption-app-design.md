...
> ⚠️ **`OBJ`·`PKG` 코드값은 패턴 추정이다.** 정확한 값은 해당 필드의 Data Element → Domain → Value Range(고정값 목록)에서 확인해야 한다. 코드에는 리터럴 대신 **상수 또는 설정 테이블 값**을 사용한다.
...
따라서 **"패키지/오브젝트 단위만 허용"은 리스크 통제 목적이 아니다.** 신청 건수 절감·일괄 정리를 위한 효율 목적이며, 오히려 가장 위험한 `PKG`를 여는 결정이다. → 6.5장의 통제 장치가 필수다.
...
> 보안 체크를 `PKG` 로 열면 해당 패키지의 보안 검증이 전부 꺼진다. 체크별 차등이 필수인 이유다.
...
| `PKG` | 신청 근거(증빙) 스냅샷. 효력은 패키지 전체 + 향후 오브젝트 | 참고용 |

```
예) 패키지 신청서
  Header : SCOPE=PKG, DEVCLASS=ZFI_LEGACY, VALID_TO=2027-12-31
...
`PKG` / `OBJ` 판정에는 **라인 정보가 들어가지 않는다.** 이것이 R2 의 기술적 실체이며, 코드 수정에 강건한 이유다.
...
| `validateScopeFields` | `PKG`면 오브젝트 필드 공란, `OBJ`면 오브젝트 필드 필수, `FND`면 위치 정보까지 필수 |
...
| 승인 권한 차등 | `OBJ` = 팀리더 / `PKG` = **아키텍트 전용** (설정 테이블) |
...
| 5 | 아이템은 `FND` 에서만 면제 대상이고, `OBJ` / `PKG` 에서는 **증빙 스냅샷**이다 |
...
| 적용범위 제어 | 검증 로직에서 판정 | **설정 테이블 매트릭스** | 체크그룹 × 범위 2차원. Phase 2 확장이 확정되어 코드 변경 없이 열고 닫아야 한다 |
...
```
  ...
  │ ztatcscope    정책 설정        │ 승인 │ 표준 ATC 가 억제              │
...
- **Phase 1**: `FND` 가 비활성 → 화면 목록에 뜨지 않고 validation 이 거부 → 요건 충족
- **Phase 2**: `PERF` / `SECURITY` 행 추가 → **코드 변경 0**
...
| 테이블 | 용도 |
|---|---|
| `ztatcexempt` | 예외 신청 헤더 (승인 대상). 키 `exemptuuid` |
| `ztatcexempti` | 신청 아이템 (근거 finding). 키 `itemuuid` |
| `ztatcexemptlog` | 상태 변경 이력. 키 `loguuid` |
| `ztatcfinding` | ATC finding 스냅샷. 키 `findinguuid` |
| `ztatccheck` | 대상 체크 마스터 (설정) |
| `ztatcscope` | 체크그룹 × 적용범위 허용 매트릭스 (설정) |
...
`createFromFinding` 액션은 패키지 스코프일 때 헤더의 오브젝트 필드를 비운다.
효력 범위가 패키지 전체임이 데이터에서도 드러나게 하려는 것이다.
...
코드를 고쳐 라인이 밀려도 `OBJ` / `PKG` 예외는 유지된다.

## 18.6 패키지 승인 통제 (구현된 것)

| 장치 | 구현 위치 |
|---|---|
| 유효기간 필수 + 설정 기반 상한 | `validateValidity` |
| `PKG` 는 더 높은 승인 레벨 | `ztatcscope-apprlevel` + `is_approver` |
| 승인 전 영향도 확인 | `simulateImpact` 액션 |
| **상신 시 영향 건수를 근거 텍스트에 자동 기입** | `submit` 액션 |
| `PKG` 행 경고색 표시 | `ScopeCriticality` |
...
| 스냅샷 인덱스 / 보관 정책 | 성능 장애 발생 후 긴급 대응 |

## 18.8 남은 확인 과제

코드는 아래 5개를 **가정**으로 두고 작성했다. 활성화 전 확인이 필요하며,
가정이 틀렸을 때 수정 범위를 최소화하도록 의존을 격리해 두었다.

| # | 가정 | 틀리면 수정 범위 |
|---|---|---|
| 1 | `ZSCM00010` 의 생성자/변경자 필드가 `ernam` / `aenam` | CDS 2개 × 2줄 + BDEF mapping 2줄 |
| 2 | `SATC_API_FINDINGS` 필드명 | `zcl_atc_finding_reader` 의 SELECT 한 곳 |
...
5번이 미해결인 동안 `zcl_atc_exempt_sync` 는 스텁이다. 그 상태로도 신청·승인·이력·조회는
전부 동작하고 CBO 대장도 채워지지만, **표준 ATC 억제는 되지 않는다** (TR 릴리즈 차단 유지).
추적 경로는 표준 Fiori 앱 `Approve ATC Exemptions` 의 OData 서비스다.
