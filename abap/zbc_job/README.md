...
**③ `workperiod` 는 아직 대응이 없다.** 달력 블록 안에서만 채워지는 것으로
보아 **작업일 규칙의 반복 간격(월)** 으로 추정된다 — "매 2개월마다, 월말에서
3번째 작업일". 그렇다면 APJ 에서는 `periodic_value` + `granularity = MONTH`,
즉 **`반복주기`(`PRDMONTHS`)와 같은 슬롯**이다. 새 파라미터가 필요한 것이
아니라 이관할 때 `PeriodMonths` 로 넣으면 된다.

> **확인 필요:** AS-IS 가 `PRDMONTHS` 와 `workperiod` 를 **동시에** 채우는
> 경우가 있는지. 있으면 같은 슬롯을 두고 부딪히므로 어느 쪽이 이기는지
> 정해야 한다.
