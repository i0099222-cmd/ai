...
```
  ...
  Jobs" / OData 서비스 →│  IF_APJ_DT/RT_EXEC_OBJECT    │─┐
                        └──────────────────────────────┘ │   ┌────────────────────┐
                                                          ├──→│ ZCL_JOB_TEST_CORE  │→ ZTJOB_PROBE
                        ┌──────────────────────────────┐ │   └────────────────────┘
  SM36 / SM37         →│  ZR_JOB_TEST (Standard ABAP)  │─┘
                        └──────────────────────────────┘
```

프로브 테이블 `ZTJOB_PROBE` 에는 `schedule_mode` (`A`=Application Job, `C`=Classic) 가
찍히므로, **한 테이블을 조회하는 것만으로 두 경로의 차이가 그대로 보인다.**

```sql
SELECT run_tag, schedule_mode, seq_no, exec_user, exec_stamp,
       user_timezone, host, is_batch, job_name, job_count
  FROM ztjob_probe ORDER BY exec_stamp
```
...
| 2  | **스텝별 실행 사용자** 지정 | O | X | SM36 Step 화면의 "User" 필드. APJ 는 스케줄한 사용자 컨텍스트로 실행 → `ZTJOB_PROBE-exec_user` 비교 |
...
| 5  | **실행 대상 서버 / 서버 그룹** 지정 | O | X | SM36 > Exec. target. `ZTJOB_PROBE-host` 로 실제 실행 서버 비교 |
...
| 14 | **실행 중 잡 중지 시점** | O (즉시 중단 시도) | ? | `p_sleep=10, p_count=5` 로 돌리고 중간에 취소 → `ZTJOB_PROBE` 에 몇 건 남았는지 비교 |
...
- `sy-batch` / `sy-host` 는 ABAP Cloud 언어버전에서 못 읽는다.
  → `ZTJOB_PROBE` 에서 `schedule_mode='A'` 행은 `host`/`is_batch` 가 비고,
    `'C'` 행만 채워진다. 이 자체가 티어 차이의 증거.
...
|----|------|----------|---------|----------|
| TC01 | `TC01-BASE-A` | Application Jobs 앱 > 즉시 실행 | count=3 | 프로브 3건, exec_user, timezone |
...
| TC05 | `TC05-CANCEL-A/C` | count=5, sleep=10 후 취소 | — | 남은 프로브 건수 비교 (#14) |
...
```sql
SELECT * FROM ztjob_probe WHERE run_tag = 'TC01-BASE-A' ORDER BY seq_no
```

또는 OData 서비스의 `Probe` 엔티티셋에서 조회.
