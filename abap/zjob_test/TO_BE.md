...
> 상태: **1차 판정 완료 / ZBCS00110 필드 확인 대기**
...
| AS-IS 요소 | Application Job 대응 | 판정 | 근거 |
|-----------|---------------------|------|------|
| **`program` 을 파라미터로 등록** | 잡 카탈로그 엔트리 **화이트리스트**만 가능 | **✗ 치명적** | APJ 는 실행 클래스가 사전 등록돼야 함 |
| **`jobclass` (A/B/C)** | 대응 파라미터 없음 | ✗ | COMPARISON #4 |
| **`jobname` 직접 지정** | 프레임워크 자동 생성 | ✗ | COMPARISON #16 |
| `lt_pg` 다중 행 (스텝 목록으로 추정) | 실행 오브젝트 1개 | ✗ *(확인 필요)* | COMPARISON #1 |
| `reqid` / `reqname` / `reqdatetime` | RAP 관리필드 + 커스텀 필드 | ○ | |
| `status` / `message` | RAP `reported` / 로그 테이블 | ○ | |

### 결론

AS-IS 는 **"임의의 프로그램을 스케줄하는 범용 잡 생성기"** 다.
Application Job Framework 는 **화이트리스트 모델**이므로 이 요구사항을 구조적으로 만족할 수 없다.
...
| AS-IS BDC | 표준 FM | 커버 |
|-----------|---------|------|
| SM36 첫 화면 | `JOB_OPEN` | `jobname`, `jobclass` 지정 가능 |
| Step 등록 | `JOB_SUBMIT` | `report`, `variant`, `authcknam`(스텝 사용자) — 스텝마다 반복 호출 |
| 시작 조건 | `JOB_CLOSE` | `strtimmed`, `sdlstrtdt`/`sdlstrttm`, `predjob`, `event_id`, 주기 옵션 |
| `_DELETE` | `BP_JOB_DELETE` | |
| `_STATUS` | `BP_JOB_SELECT` / `BP_JOB_READ` / TBTCO·TBTCP | |
...
| # | 확인할 것 | 왜 필요한가 |
|---|----------|-------------|
| 1 | **`ZBCS00110` 필드 목록** | 시작조건(즉시/예약/주기/**이벤트**/**선행 잡**/대상 서버)이 뭐가 있는지 → `JOB_CLOSE` 파라미터 매핑 확정. 이벤트·선행 잡이 있으면 APJ 불가 항목 추가 (#6, #7) |
| 2 | **`ZBCS0012` 필드 목록** | `lt_pg` 가 정말 스텝 목록인지 → 다중 스텝 여부 확정 (#1) |
