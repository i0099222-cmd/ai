...
| `module_msg_key` | `MessageId` | 메시지 코드. ⬜ 타입 확인 필요 |
...
| 3 | `module_msg_key` 의 타입 (현재 `char30` 가정) | 뷰 필드 목록 | 테이블 2곳 + 인터페이스 2곳 |
| 4 | **`create_exemption` 의 `i_check_class` / `i_check_code` 타입** 🔴 | 메소드 시그니처 | GUID 를 받으면 그대로. 클래스명(CHAR)을 받으면 변환 필요 |
