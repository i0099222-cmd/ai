...
**5초 같은 짧은 값은 안 된다.** 두 잡이 같은 순간에 시작되는 게 아니라
스케줄러 릴리스와 잡 시작 오버헤드로 몇 초는 어긋난다. 3분이면 SM12 를 볼 여유도 생긴다.
테스트가 끝나면 다시 주석 처리한다.

`WAIT` 는 암시적 커밋을 일으키지만 잠금은 안 풀린다. `iv_scope = '1'` 이라서다.
ABAP Cloud 구문 검사에서 `WAIT` 가 걸리면 `DO 50000000 TIMES. ENDDO.` 로 대신하고,
한 번 돌려서 3분쯤 되게 횟수를 맞춘다.
