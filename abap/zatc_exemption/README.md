...
표준의 모델은 **신청 시점에 행이 생기고, 승인은 그 행의 상태를 바꾸는 것**이다
(`SATC_CI_R_EXEMPTION` 의 `state` / `approver` 변경). 우리도 같은 시점에 맞춘다.

| 우리 상태 | 표준 저장소 | 호출 |
|---|---|---|
| 상신 `20` | 승인대기로 생성 | `create_exemption( )` + `send_to_approver( )` |
| 승인 `30` | 승인 — **여기서 ATC 차단이 풀린다** | `approve_exemption_by_id( )` |
| 반려 `40` / 철회 `50` / 만료 `60` | 무효화 | `reject_exemptions_by_id( )` |
| 상신철회 → `10` | 무효화 + `extexemptid` 비움 | `reject_exemptions_by_id( )` |

상신철회에서 ID 를 비우는 이유: 안 비우면 재상신 때 "이미 등록됨"으로 보고
건너뛰어, 표준에는 반려된 행만 남고 영영 승인되지 않는다.

승인 시 `extexemptid` 가 비어 있으면(상신 때 표준 등록이 실패한 건) 한 번 더
생성한 뒤 승인한다. 이 보정이 없으면 대장은 승인인데 ATC 는 계속 막는 상태로 굳는다.

**호출이 액션이 아니라 `save_modified` 에 있는 이유**는 남의 DB 를 바꾸고 잠금을
잡는 작업이기 때문이다. 액션(interaction phase)에서 부르면 뒤의 검증이 실패해도
표준 예외는 이미 만들어져 있고, RAP 이 버퍼를 롤백해도 표준은 되돌아가지 않는다.
