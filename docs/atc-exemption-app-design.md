...
| SCOPETYPE | 아이템의 역할 |
|---|---|
| `FND` | 면제 대상 그 자체 (1:1). 아이템의 `checksum` 이 판정에 쓰인다 |
...
| 항목 | 미루면 |
|---|---|
| 아이템의 `lineno` / `checksum` 컬럼 | 운영 데이터 있는 상태에서 구조변경 + CDS/BDEF 수정 + 마이그레이션 + 재테스트 |
...
| 4 | `SATC_API_FINDINGS` 의 `checksum` 필드 존재 여부 | 뷰 열기 | 리더 + 뷰 + 아이템 테이블 |
| 5 | 컨트롤러가 스코프 파라미터를 받는지 (건별 생성만 되는지) | 3번과 함께 확인 | 건별만 되면 예외 ID 가 헤더가 아니라 아이템으로 내려간다 |
