...
"! 런처가 ZTJOB_RUN-EXEC_CLASS 에 적힌 이름으로 동적 생성해서 EXECUTE 를 부른다.
"!
"! ** 왜 리포트가 아니라 클래스인가 **
...
INTERFACE zif_bc_job_step
  PUBLIC.

  "! 배치 실행 본체.
  "!
  "! @parameter iv_param | ZTJOB_RUN-PARAM 의 app 영역. 업무 파라미터를 담은 JSON.
  "!                       배리언트를 대신하는 자리다.
  "! @parameter rv_message | 잡 로그에 남길 결과 요약
  "! @raising zcx_bc_job | 실행 실패. 던지면 잡이 오류 종료된다.
  METHODS execute
    IMPORTING
